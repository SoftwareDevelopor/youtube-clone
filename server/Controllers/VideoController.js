...
exports.downloadAndSaveVideo = async (req, res) => {
  ...
  try {
    ...
    const thumbFilename = `${Date.now()}-${videotitle.replace(/\s+/g, '_')}-thumb.${thumbExt}`;
    ...
    await file.save();
    return res.status(201).json({ message: 'Video and thumbnail downloaded and saved', video: file });
  } catch (error) {
    console.log(error);
    return res.status(500).json({ message: 'Failed to download and save video/thumbnail' });
  }
};
